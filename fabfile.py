...
def staging():
    ...
    env.hosts = ['deploy@10.0.1.35', 'deploy@10.0.1.36'] #private IP to staging server
    env.app_directory = "/opt/local/share/file-parse"
    env.virtualenv_root = "%s/venv" % env.app_directory
    env.activate = "source %s/bin/activate" % env.virtualenv_root
    env.role = "staging"
...
def deploy_app():
    ...
    project.rsync_project(local_dir="fileparse requirements bin conf", remote_dir=env.app_directory, exclude=["*~", "*.pyc"])
